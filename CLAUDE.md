...
lib/api/            — Cliente OpenRouter BYOK (openrouter.ts, keyStorage, generators, pricing)
lib/validators/     — Validadores client-side (codeValidator, executionValidator)
...
**Executar antes de qualquer build Android.** (A pipeline de IA roda client-side via OpenRouter — não há mais geração server-side.)
...
| Tabela | Propósito |
|--------|-----------|
| `generated_apps` | Spells: id, name, code (HTML), currentVersion, iconPath, totalSpendUsd |
...
| `webview_ai_cache` | Cache de resultados de AI chamados dentro da WebView (campo `costUsd`) |
...
**Root layout** (`app/_layout.tsx`): handlers de notificação, ShareReceiver, modais BYOK (CostEstimate, KeyMissing).
...
| Bridge UI store | `lib/bridgeUIStore.ts` | Refs de WebView, `costEstimateRequest`, `keyMissingRequest` (modais BYOK) |
