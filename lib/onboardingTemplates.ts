...
export function shoppingListTemplate(t: T) {
  ...
  const langHint = jsEsc(`Respond in the language matching locale "${getCurrentLanguage()}". `);
  return {
    name: title,
...
function suggestItems(){if(typeof AppacadabraAI==='undefined'){ console.warn('AI bridge missing'); return; }var sg=document.getElementById('suggestions');sg.innerHTML='<div class="suggest-label"><span class="loading"></span> ${jsEsc(t('tplAIThinking'))}</div>';var recent=items.map(function(it){return it.n}).join(', ');var prompt='${langHint}Given this shopping list: ['+recent+']. Suggest 5 more items the user might need. Reply with a JSON object containing an "items" array of strings. Example: {"items":["Milk","Bread","Eggs"]}';window.handleAISuggest=function(success,result){console.log('AI Suggest Success:', success);if(!success){sg.innerHTML='';return}try{var arr=result.items||result;sg.innerHTML=arr.map(function(s){return '<span class=\"chip\" onclick=\"addSuggestion(this)\">+ '+s+'</span>'}).join('');localStorage.setItem('shop_ai_cache',JSON.stringify(arr))}catch(e){console.error('AI Suggest Parse Error:', e);sg.innerHTML=''}};var cached=localStorage.getItem('shop_ai_cache');if(cached&&items.length<2){try{window.handleAISuggest(true,JSON.parse(cached))}catch(e){}return}AppacadabraAI.withSchema({type:'object',properties:{items:{type:'array',items:{type:'string'}}},required:['items']}).generate(prompt,'handleAISuggest')}
...
.ex .del-btn{padding:6px 10px;border:none;border-radius:8px;background:transparent;color:var(--dim);font-size:1rem;cursor:pointer}
...
function delEx(i){exercises.splice(i,1);vib(30);save()}
function save(){localStorage.setItem('workout_ex',JSON.stringify(exercises));render()}
function render(){var el=document.getElementById('exList');el.innerHTML=exercises.map(function(e,i){return '<div class="ex"><div class="row"><span class="name">'+e.n+'</span><div style="display:flex;gap:4px"><button class="done-btn'+(e.d?' completed':'')+'" onclick="toggleEx('+i+')">'+(e.d?'✓':'${jsEsc(t('tplWorkoutDone'))}')+'</button><button class="del-btn" onclick="delEx('+i+')">✕</button></div></div></div>'}).join('')}
function loadHealth(){if(typeof AppacadabraHealth==='undefined'){showToast('${jsEsc(t('tplHealthUnavailable'))}'); console.warn('Health missing'); return}var now=Date.now();var dayAgo=now-86400000;var banner=document.getElementById('healthBanner');banner.style.display='flex';banner.innerHTML='<span class="loading"></span>';window.handleSteps=function(success,result){console.log('Steps Success:', success);if(!success)return;try{var d=result;healthData=healthData||{};healthData.steps=d.totalSteps||0;renderHealth()}catch(e){console.error('Steps Parse Error:', e)}};window.handleCalories=function(success,result){console.log('Calories Success:', success);if(!success)return;try{var d=result;healthData=healthData||{};healthData.calories=Math.round(d.totalCalories||0);renderHealth()}catch(e){console.error('Calories Parse Error:', e)}};AppacadabraHealth.getSteps(dayAgo,now,'handleSteps');AppacadabraHealth.getCalories(dayAgo,now,'handleCalories')}
...
function generatePlan(){if(typeof AppacadabraAI==='undefined'){ console.warn('AI missing'); return; }var plan=document.getElementById('aiPlan');plan.innerHTML='<div class="ai-plan"><div class="suggest-label"><span class="loading"></span> ${jsEsc(t('tplAIThinking'))}</div></div>';var context='${langHint}Generate a workout plan with exercise names in the user native language.';if(healthData){context+=' User health today: '+healthData.steps+' steps, '+healthData.calories+' calories burned.'}context+=' Return a JSON object with an "exercises" array. Example: {"exercises":[{"name":"Push-ups","sets":"3x15"}]}';window.handleAIPlan=function(success,result){console.log('AI Plan Success:', success);if(!success){plan.innerHTML='';return}try{var arr=result.exercises||result;plan.innerHTML='<div class="ai-plan"><h3>🤖 ${jsEsc(t('tplAIPlan'))}</h3>'+arr.map(function(e){return '<div class=\"ex-item\" onclick=\"addExFromPlan(\\''+e.name.replace(/'/g,"\\\\'")+'\\')\">'+(e.name)+' — '+(e.sets||'')+' <span style=\"color:var(--dim)\">+</span></div>'}).join('')+'</div>';localStorage.setItem('workout_ai_cache',JSON.stringify(arr))}catch(e){console.error('AI Plan Parse Error:', e);plan.innerHTML=''}};var cached=localStorage.getItem('workout_ai_cache');if(cached&&!healthData){try{window.handleAIPlan(true,JSON.parse(cached))}catch(e){}return}AppacadabraAI.withSchema({type:'object',properties:{exercises:{type:'array',items:{type:'object',properties:{name:{type:'string'},sets:{type:'string'}},required:['name','sets']}}},required:['exercises']}).generate(context,'handleAIPlan')}
...
function scanReceipt(){if(typeof AppacadabraCamera==='undefined'){ console.warn('Camera bridge missing'); return; }window.handleReceiptPhoto=function(success,base64){console.log('Camera Photo Success:', success);if(!success)return;showToast('${jsEsc(t('tplAIThinking'))}');window.handleReceiptAI=function(ok,result){console.log('Receipt AI Success:', ok);if(!ok)return;try{var items=result.items||result;items.forEach(function(it){addParsedExpense(it.description,it.amount,it.category||'📦')})}catch(e){console.error('Receipt parse error:',e)}};AppacadabraAI.fromImage(base64).withSchema({type:'object',properties:{items:{type:'array',items:{type:'object',properties:{description:{type:'string'},amount:{type:'number'},category:{type:'string',enum:['🍔','🚗','🏠','🎮','🛍️','📦']}},required:['description','amount']}}},required:['items']}).generate('${langHint}Extract expense items from this receipt. Return a JSON object with an \"items\" array containing description (in the user native language), amount (number), and category emoji.','handleReceiptAI')};AppacadabraCamera.takePhoto('handleReceiptPhoto')}
function toggleVoice(){if(typeof AppacadabraAudio==='undefined'){ console.warn('Audio bridge missing'); return; }var btn=document.getElementById('micBtn');if(!isRecording){isRecording=true;btn.classList.add('recording');btn.textContent='⏹️ ${jsEsc(t('tplStopRecording'))}';window.handleRecordStart=function(success){console.log('Record Start Success:', success);if(!success){isRecording=false;btn.classList.remove('recording');btn.textContent='🎙️ ${jsEsc(t('tplVoiceInput'))}'}};AppacadabraAudio.recordStart('handleRecordStart')}else{isRecording=false;btn.classList.remove('recording');btn.textContent='🎙️ ${jsEsc(t('tplVoiceInput'))}';window.handleAudioRecorded=function(success,base64){console.log('Audio Recorded Success:', success);if(!success)return;showToast('${jsEsc(t('tplAIThinking'))}');window.handleAudioParse=function(ok,result){console.log('Audio Parse Success:', ok);if(!ok)return;try{var items=result.items||result;items.forEach(function(it){addParsedExpense(it.description,it.amount,it.category||'📦')})}catch(e){console.error('Audio parse error:',e)}};AppacadabraAI.fromAudio(base64).withSchema({type:'object',properties:{items:{type:'array',items:{type:'object',properties:{description:{type:'string'},amount:{type:'number'},category:{type:'string',enum:['🍔','🚗','🏠','🎮','🛍️','📦']}},required:['description','amount']}}},required:['items']}).generate('${langHint}The user is dictating expense items. Extract each expense mentioned with a JSON object containing an \"items\" array with description (in the user native language), amount (number), and category emoji. If amounts are unclear, estimate.','handleAudioParse')};AppacadabraAudio.recordStop('handleAudioRecorded')}}
render();
</script>
</body></html>`
  };
