...
function suggestItems(){if(typeof AppacadabraAI==='undefined'){ console.warn('AI bridge missing'); return; }var sg=document.getElementById('suggestions');sg.innerHTML='<div class="suggest-label"><span class="loading"></span> ${jsEsc(t('tplAIThinking'))}</div>';var recent=items.map(function(it){return it.n}).join(', ');var prompt='${langHint}Given this shopping list: ['+recent+']. Suggest 5 more items the user might need. Reply with a JSON array of strings only, no explanation, in the user native language. Example: ["Milk","Bread","Eggs","Butter","Cheese"]';window.handleAISuggest=function(success,result){console.log('AI Suggest Success:', success);if(!success){sg.innerHTML='';return}try{var arr=result;sg.innerHTML=arr.map(function(s){return '<span class=\"chip\" onclick=\"addSuggestion(this)\">+ '+s+'</span>'}).join('');localStorage.setItem('shop_ai_cache',JSON.stringify(arr))}catch(e){console.error('AI Suggest Parse Error:', e);sg.innerHTML=''}};var cached=localStorage.getItem('shop_ai_cache');if(cached&&items.length<2){try{window.handleAISuggest(true,JSON.parse(cached))}catch(e){}return}AppacadabraAI.withSchema({type:'array',items:{type:'string'}}).generate(prompt,'handleAISuggest')}
...
function generatePlan(){if(typeof AppacadabraAI==='undefined'){ console.warn('AI missing'); return; }var plan=document.getElementById('aiPlan');plan.innerHTML='<div class="ai-plan"><div class="suggest-label"><span class="loading"></span> ${jsEsc(t('tplAIThinking'))}</div></div>';var context='${langHint}Generate a workout plan with exercise names in the user native language.';if(healthData){context+=' User health today: '+healthData.steps+' steps, '+healthData.calories+' calories burned.'}context+=' Return JSON array of 6 exercise objects with name and sets fields. Example: [{"name":"Push-ups","sets":"3x15"},{"name":"Squats","sets":"3x20"}]';window.handleAIPlan=function(success,result){console.log('AI Plan Success:', success);if(!success){plan.innerHTML='';return}try{var arr=result;plan.innerHTML='<div class="ai-plan"><h3>🤖 ${jsEsc(t('tplAIPlan'))}</h3>'+arr.map(function(e){return '<div class=\"ex-item\" onclick=\"addExFromPlan(\\''+e.name.replace(/'/g,"\\\\'")+'\\')\">'+(e.name)+' — '+(e.sets||'')+' <span style=\"color:var(--dim)\">+</span></div>'}).join('')+'</div>';localStorage.setItem('workout_ai_cache',JSON.stringify(arr))}catch(e){console.error('AI Plan Parse Error:', e);plan.innerHTML=''}};var cached=localStorage.getItem('workout_ai_cache');if(cached&&!healthData){try{window.handleAIPlan(true,JSON.parse(cached))}catch(e){}return}AppacadabraAI.withSchema({type:'array',items:{type:'object',properties:{name:{type:'string'},sets:{type:'string'}},required:['name','sets']}}).generate(context,'handleAIPlan')}
