...
function doSearch(q){
  ...
  window.handleContactSearch=function(success,result){
    if(!success)return;
    try{
      allContacts=typeof result==='string'?JSON.parse(result):result;
      renderContacts();
