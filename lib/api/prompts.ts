/**
 * Client port of `firebase/functions/src/prompts.ts`.
 *
 * The prompt strings must match the server byte-for-byte — output quality
 * depends on prompt identity. `lib/__tests__/prompts.parity.test.ts` reads
 * the server file at test time and fails CI on any drift.
 *
 * `SYSTEM_PREAMBLE` lives in `lib/api/systemPrompt.ts` (the parity test for it
 * runs there).
 */

// Smart patching instructions (still used by the convert path).
