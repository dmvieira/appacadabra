# Play Store Translations

## English (en-US) - Default

### App Name (30 chars)
Appacadabra: AI Tool Generator

### Short Description (80 chars)
Turn your desire into functional tools instantly. No code required, just AI

### Full Description (4000 chars)
**What do you need help with?** Describe it. The AI creates it. 🪄

Appacadabra is an **AI Tool Generator** that turns your ideas into working utilities in seconds. No coding required—just describe what you need and watch the magic happen.

Need a habit tracker? A workout timer? A budget calculator? A study planner? A new game just for you? Just type what you want and the AI builds it for you, instantly.

**✨ Key Features:**

• **Describe & Create**: Just type "A Pomodoro timer with a space theme" or "A tip calculator for groups" and get a fully functional tool in seconds.
• **Persistent & Private**: Your spells are stored locally on your device. They work offline and are completely yours.
• **Native Device Powers**: Unlike basic web wrappers, your spells can access real device features:
  📅 Calendar • 🔔 Notifications • 📍 Location • 👤 Contacts • 🦿 Health Connect • 🤖 AI • 🧭 Sensors • 🔒 Biometric Lock • 🏠 Shortcuts • 📷 Camera & QR • 🗣️ TTS • 📳 Haptics • 🎨 Image Gen • 🎤 Speech Recog • 🔋 Battery • 🖨️ Printing
• **Open Other Apps**: Your spells can launch WhatsApp, Telegram, Notion, Spotify—any app that supports sharing or deep links!
• **📥 Share TO Spells**: Share files directly from other apps for instant processing:
  📱 WhatsApp exports → Summarize key topics
  📄 PDFs → Extract important data
  🎤 Audio → Transcribe & analyze
  📸 Images → Extract text or generate reports
• **Edit with AI**: Want to change something? Just say "Make the background blue" or "Add a reset button," and the AI updates it in real-time.
• **Share Your Spells**: Export your creations to share with friends—your data stays private.

**🔒 Privacy First:**
We believe in user ownership. Your spells live on your device. We don't resell your data or claim ownership of your creations.

**Download Appacadabra today and start building your own spell book, one spell at a time!**

---

## Português (pt-BR)

### App Name
Appacadabra: Gerador de Ferramentas

### Short Description
Transforme desejos em ferramentas funcionais. Sem código, apenas IA

### Full Description
**No que você precisa de ajuda?** Descreva. A IA cria. 🪄

Appacadabra é um **Gerador de Ferramentas com IA** que transforma suas ideias em utilidades funcionais em segundos. Sem programação—apenas descreva o que você precisa e veja a mágica acontecer.

Precisa de um rastreador de hábitos? Timer de treino? Calculadora de orçamento? Planejador de estudos? Um jogo novo só pra você? Digite o que você quer e a IA constrói para você, instantaneamente.

**✨ Recursos Principais:**

• **Descreva & Crie**: Digite "Um timer Pomodoro com tema espacial" e receba uma ferramenta funcional em segundos.
• **Persistente & Privado**: Seus feitiços são armazenados localmente. Funcionam offline e são completamente seus.
• **Poderes Nativos**: Diferente de wrappers web básicos, seus feitiços podem acessar recursos reais do dispositivo:
  📅 Calendário • 🔔 Notificações • 📍 Localização • 👤 Contatos • 🦿 Health Connect • 🤖 IA • 🧭 Sensores • 🔒 Bloqueio Biométrico • 🏠 Atalhos • 📷 Câmera & QR • 🗣️ Texto para Fala • 📳 Vibração • 🎨 Geração de Imagem • 🎤 Reconhecimento de Voz • 🔋 Bateria • 🖨️ Impressão
• **Abra Outros Apps**: Seus feitiços podem abrir WhatsApp, Telegram, Notion, Spotify—qualquer app com deep links!
• **📥 Compartilhe COM Feitiços**: Compartilhe arquivos de outros apps para processamento instantâneo:
  📱 Exportações do WhatsApp → Resuma os principais tópicos
  📄 PDFs → Extraia dados importantes
  🎤 Áudio → Transcreva e analise
  📸 Imagens → Extraia texto ou gere relatórios
• **Edite com IA**: Quer mudar algo? Diga "Mude o fundo para azul" ou "Adicione um botão de reset" e a IA atualiza na hora.
• **Compartilhe**: Exporte suas criações para compartilhar com amigos—seus dados ficam privados.

**🔒 Privacidade Primeiro:**
Acreditamos na propriedade do usuário. Seus feitiços ficam no seu dispositivo. Não vendemos seus dados nem reivindicamos propriedade das suas criações.

**Baixe Appacadabra hoje e comece a construir seu próprio livro de feitiços!**

---

## Español (es-ES)

### App Name
Appacadabra: Generador IA

### Short Description
Convierte tus deseos en herramientas funcionales. Sin código, solo IA

### Full Description
**¿Con qué necesitas ayuda?** Descríbelo. La IA lo crea. 🪄

Appacadabra es un **Generador de Herramientas con IA** que convierte tus ideas en utilidades funcionales en segundos. Sin programar—solo describe lo que necesitas y mira la magia suceder.

¿Necesitas un rastreador de hábitos? ¿Timer de ejercicio? ¿Calculadora de presupuesto? ¿Un juego nuevo? Escribe lo que quieres y la IA lo construye para ti, al instante.

**✨ Características Principales:**

• **Describe y Crea**: Escribe "Un timer Pomodoro con tema espacial" y obtén una herramienta funcional en segundos.
• **Persistente y Privado**: Tus hechizos se guardan localmente. Funcionan offline y son completamente tuyos.
• **Poderes Nativos**: Accede a funciones reales del dispositivo:
  📅 Calendario • 🔔 Notificaciones • 📍 Ubicación • 👤 Contactos • 🦿 Health Connect • 🤖 IA • 🧭 Sensores • 📳 Vibración • 🔒 Bloqueo Biométrico • 🏠 Atajos • 📷 Cámara y QR • 🗣️ Texto a Voz • 🎨 Gen. Imágenes • 🎤 Voz • 🔋 Batería • 🖨️ Impresión
• **Abre Otras Apps**: ¡Lanza WhatsApp, Telegram, Notion, Spotify—cualquier app con deep links!
• **📥 Comparte CON Hechizos**: Comparte archivos de otras apps para procesamiento instantáneo:
  📱 Exportaciones de WhatsApp → Resume los temas clave
  📄 PDFs → Extrae datos importantes
  🎤 Audio → Transcribe y analiza
  📸 Imágenes → Extrae texto o genera reportes
• **Edita con IA**: Di "Cambia el fondo a azul" o "Agrega un botón de reset" y la IA lo actualiza al instante.
• **Comparte**: Exporta tus creaciones—tus datos permanecen privados.

**🔒 Privacidad Primero:**
Tus hechizos viven en tu dispositivo. No vendemos tus datos ni reclamamos propiedad.

**¡Descarga Appacadabra hoy y empieza a construir tu propio libro de hechizos!**

---

## Français (fr-FR)

### App Name
Appacadabra: Générateur IA

### Short Description
Transformez vos désirs en outils fonctionnels. Sans code, juste l'IA

### Full Description
**De quoi avez-vous besoin ?** Décrivez-le. L'IA le crée. 🪄

Appacadabra est un **Générateur d'Outils IA** qui transforme vos idées en utilitaires fonctionnels en quelques secondes. Sans code—décrivez simplement ce dont vous avez besoin et regardez la magie opérer.

Besoin d'un suivi d'habitudes ? Timer d'entraînement ? Calculatrice de budget ? Un jeu ? Tapez ce que vous voulez et l'IA le construit pour vous, instantanément.

**✨ Fonctionnalités Principales :**

• **Décrivez & Créez**: Tapez « Un timer Pomodoro avec un thème spatial » et obtenez un outil fonctionnel en secondes.
• **Persistant & Privé**: Vos sorts sont stockés localement. Ils fonctionnent hors ligne et vous appartiennent.
• **Pouvoirs Natifs**: Accédez aux vraies fonctionnalités de l'appareil:
  📅 Calendrier • 🔔 Notifications • 📍 Localisation • 👤 Contacts • 🦿 Health Connect • 🤖 IA • 🧭 Capteurs • 🔒 Verrou Biométrique • 🏠 Raccourcis • 📷 Caméra & QR • 🗣️ Synthèse Vocale • 📳 Haptique • 🎨 Gen. Image • 🎤 Reconnaissance Vocale • 🔋 Batterie • 🖨️ Impression
• **Ouvrez D'autres Apps**: Lancez WhatsApp, Telegram, Notion, Spotify—toute app avec deep links !
• **📥 Partagez VERS vos Sorts**: Partagez des fichiers d'autres apps pour un traitement instantané :
  📱 Exports WhatsApp → Résumez les sujets clés
  📄 PDFs → Extrayez les données importantes
  🎤 Audio → Transcrivez et analysez
  📸 Images → Extrayez du texte ou générez des rapports
• **Modifiez avec l'IA**: Dites « Mettez le fond en bleu » et l'IA met à jour instantanément.
• **Partagez**: Exportez vos créations—vos données restent privées.

**🔒 Vie Privée D'abord:**
Vos sorts restent sur votre appareil. Nous ne vendons pas vos données.

**Téléchargez Appacadabra aujourd'hui !**

---

## Deutsch (de-DE)

### App Name
Appacadabra: KI-Generator

### Short Description
Verwandle Wünsche sofort in Tools. Kein Code erforderlich, nur KI

### Full Description
**Wobei brauchst du Hilfe?** Beschreibe es. Die KI erschafft es. 🪄

Appacadabra ist ein **KI-Tool-Generator**, der deine Ideen in funktionierende Hilfsmittel in Sekunden verwandelt. Ohne Programmieren—beschreibe einfach, was du brauchst und sieh die Magie geschehen.

Brauchst du einen Habit-Tracker? Workout-Timer? Budget-Rechner? Ein Spiel? Tippe, was du willst und die KI baut es für dich, sofort.

**✨ Hauptfunktionen:**

• **Beschreiben & Erstellen**: Tippe "Ein Pomodoro-Timer mit Weltraum-Thema" und erhalte ein funktionierendes Tool in Sekunden.
• **Persistent & Privat**: Deine Zauber werden lokal gespeichert. Sie funktionieren offline und gehören dir.
• **Native Kräfte**: Zugriff auf echte Gerätefunktionen:
  📅 Kalender • 🔔 Benachrichtigungen • 📍 Standort • 👤 Kontakte • 🦿 Health Connect • 🤖 KI • 🧭 Sensoren • 🔒 Biometrische Sperre • 🏠 Verknüpfungen • 📷 Kamera & QR • 🗣️ Text-zu-Sprache • 📳 Haptik • 🎨 Bildgen. • 🎤 Spracherkennung • 🔋 Batterie • 🖨️ Drucken
• **Andere Apps Öffnen**: Starte WhatsApp, Telegram, Notion, Spotify—jede App mit Deep Links!
• **📥 Teile MIT Zaubern**: Teile Dateien von anderen Apps für sofortige Verarbeitung:
  📱 WhatsApp-Exporte → Fasse Schlüsselthemen zusammen
  📄 PDFs → Extrahiere wichtige Daten
  🎤 Audio → Transkribiere & analysiere
  📸 Bilder → Extrahiere Text oder erstelle Berichte
• **Mit KI Bearbeiten**: Sage "Mache den Hintergrund blau" und die KI aktualisiert sofort.
• **Teilen**: Exportiere deine Kreationen—deine Daten bleiben privat.

**🔒 Privatsphäre Zuerst:**
Deine Zauber bleiben auf deinem Gerät. Wir verkaufen deine Daten nicht.

**Lade Appacadabra heute herunter!**

---

## Italiano (it-IT)

### App Name
Appacadabra: Generatore IA

### Short Description
Trasforma i desideri in strumenti funzionali. Niente codice, solo IA

### Full Description
**Di cosa hai bisogno?** Descrivilo. L'IA lo crea. 🪄

Appacadabra è un **Generatore di Strumenti IA** che trasforma le tue idee in utilità funzionanti in pochi secondi. Senza programmare—descrivi semplicemente ciò di cui hai bisogno e guarda la magia accadere.

Ti serve un tracker di abitudini? Timer allenamento? Calcolatrice budget? Un gioco? Scrivi cosa vuoi e l'IA lo costruisce per te, all'istante.

**✨ Funzionalità Principali:**

• **Descrivi & Crea**: Scrivi "Un timer Pomodoro a tema spaziale" e ottieni uno strumento funzionante in secondi.
• **Persistente & Privato**: I tuoi incantesimi sono salvati localmente. Funzionano offline e sono completamente tuoi.
• **Poteri Nativi**: Accedi alle vere funzionalità del dispositivo:
  📅 Calendario • 🔔 Notifiche • 📍 Posizione • 👤 Contatti • 🦿 Health Connect • 🤖 IA • 🧭 Sensori • 🔒 Blocco Biometrico • 🏠 Scorciatoie • 📷 Fotocamera & QR • 🗣️ Testo-in-Voce • 📳 Haptics • 🎨 Gen. Immagini • 🎤 Riconoscimento Vocale • 🔋 Batteria • 🖨️ Stampa
• **Apri Altre App**: Lancia WhatsApp, Telegram, Notion, Spotify—qualsiasi app con deep links!
• **📥 Condividi CON gli Incantesimi**: Condividi file da altre app per elaborazione istantanea:
  📱 Esportazioni WhatsApp → Riassumi gli argomenti chiave
  📄 PDF → Estrai dati importanti
  🎤 Audio → Trascrivi e analizza
  📸 Immagini → Estrai testo o genera report
• **Modifica con IA**: Di' "Cambia lo sfondo in blu" e l'IA aggiorna subito.
• **Condividi**: Esporta le tue creazioni—i tuoi dati restano privati.

**🔒 Privacy Prima:**
I tuoi incantesimi restano sul tuo dispositivo. Non vendiamo i tuoi dati.

**Scarica Appacadabra oggi!**
---

## 日本語 (ja-JP)

### App Name
Appacadabra: AIツールジェネレーター

### Short Description
あなたの願いを即座に機能するツールへ。コード不要、AIだけ。

### Full Description
**何かお困りですか？** 説明するだけで、AIが作成します。🪄

Appacadabraは、あなたのアイデアを数秒で機能するツールに変える**AIツールジェネレーター**です。プログラミングは不要。必要なものを説明するだけで、魔法のように生成されます。

習慣トラッカー、筋トレタイマー、予算計算機、学習プランナー、自分だけのゲームが必要ですか？欲しいものを入力するだけで、AIが即座に作成します。

**✨ 主な機能:**

*   **説明して作成**: 「宇宙テーマのポモドーロタイマー」や「割り勘計算機」と入力するだけで、数秒で機能するツールが手に入ります。
*   **保存＆プライベート**: 生成された「魔法（ツール）」はデバイスにローカル保存されます。オフラインで動作し、完全にあなたのものです。
*   **ネイティブ機能**: 一般的なウェブアプリとは異なり、あなたの魔法はデバイスの機能にアクセスできます：
    📅 カレンダー • 🔔 通知 • 📍 位置情報 • 👤 連絡先 • 🦿 ヘルスコネクト • 🤖 AI • 🧭 センサー • 🔒 生体認証 • 🏠 ショートカット • 📷 カメラ & QR • 🗣️ 読み上げ • 📳 触覚 • 🎨 画像生成 • 🎤 音声認識 • 🔋 バッテリー • 🖨️ 印刷
*   **他のアプリを開く**: WhatsApp、Telegram、Notion、Spotifyなど、ディープリンク対応アプリを起動可能！
*   **📥 魔法へ共有 (Share TO Spells)**: 他のアプリからファイルを直接共有して即座に処理：
    📱 WhatsAppのエクスポート → 会話の要約
    📄 PDF → 重要データの抽出
    🎤 音声 → 文字起こしと分析
    📸 画像 → テキスト抽出やレポート作成
*   **AIで編集**: 変えたい箇所がありますか？「背景を青にして」や「リセットボタンを追加して」と言うだけで、AIがリアルタイムに更新します。
*   **共有**: 作った魔法をエクスポートして友達に共有できます（データはプライベートのまま）。

**🔒 プライバシー第一:**
ユーザーの所有権を尊重します。魔法はあなたのデバイス上に存在します。データを販売したり、作成物の所有権を主張したりすることはありません。

**今すぐAppacadabraをダウンロードして、あなただけの魔法の書を作り始めましょう！**

---

## 中文 (zh-CN)

### App Name
Appacadabra: AI工具生成器

### Short Description
将您的愿望立即转化为实用工具。无需代码，仅需AI。

### Full Description
**你需要什么帮助？** 描述一下。AI 就会为你创造。🪄

Appacadabra 是一款**AI工具生成器**，能在几秒钟内将你的想法转化为实用的工具。无需编程——只需描述你的需求，就能见证奇迹发生。

需要习惯追踪器？健身计时器？预算计算器？学习计划表？还是为你量身定制的游戏？只需输入你想要的，AI 就会立刻为你构建。

**✨ 主要功能:**

*   **描述即创造**: 只需输入“以太空为主题的番茄钟”或“AA制计算器”，几秒钟内就能获得完全可用的工具。
*   **本地保存 & 隐私保护**: 你的“魔法（工具）”存储在本地设备上。离线可用，完全属于你。
*   **原生设备能力**: 与普通的网页应用不同，你的魔法可以访问真实的设备功能：
    📅日历 • 🔔通知 • 📍位置 • 👤联系人 • 🦿健康连接 (Health Connect) • 🤖AI • 🧭传感器 • 🔒生物识别锁 • 🏠桌面快捷方式 • 📷相机 & QR • 🗣️文本转语音 • 📳触觉 • 🎨图像生成 • 🎤语音识别 • 🔋电池 • 🖨️打印
*   **打开其它应用**: 你的魔法可以启动 WhatsApp, Telegram, Notion, Spotify——任何支持深度链接的应用！
*   **📥 分享到魔法 (Share TO Spells)**: 从其它应用直接分享文件进行即时处理：
    📱 WhatsApp 聊天记录 →总结关键话题
    📄 PDF →提取重要数据
    🎤 音频 →转录和分析
    📸 图片 →提取文本或生成报告
*   **AI辅助编辑**: 想要修改什么？只需说“把背景改成蓝色”或“添加重置按钮”，AI 会实时更新。
*   **分享**: 导出你的作品与朋友分享——你的数据始终保密。

**🔒 隐私优先:**
我们尊重用户所有权。你的魔法存在于你的设备上。我们不出售你的数据，也不主张对你创作的所有权。

**立即下载 Appacadabra，开始构建你自己的魔法书吧！**

---

## 한국어 (ko-KR)

### App Name
Appacadabra: AI 도구 생성기

### Short Description
당신의 아이디어를 즉시 기능적인 도구로. 코딩 불필요, 오직 AI.

### Full Description
**무엇이 필요하신가요?** 설명만 하세요. AI가 만들어 드립니다. 🪄

Appacadabra는 당신의 아이디어를 순식간에 작동하는 도구로 바꿔주는 **AI 도구 생성기**입니다. 코딩은 전혀 필요 없습니다. 필요한 것을 설명하기만 하면 마법처럼 만들어집니다.

습관 추적기, 운동 타이머, 예산 계산기, 학습 플래너, 나만의 게임이 필요하신가요? 원하는 것을 입력하기만 하면 AI가 즉시 제작해 드립니다.

**✨ 주요 기능:**

• **설명하고 만들기**: "우주 테마의 포모도로 타이머" 또는 "더치페이 계산기"라고 입력하면 몇 초 만에 완벽하게 작동하는 도구를 얻을 수 있습니다.
• **저장 및 개인정보 보호**: 생성된 '마법(도구)'은 기기에 로컬로 저장됩니다. 오프라인에서도 작동하며 완전히 당신의 것입니다.
• **네이티브 기능**: 단순한 웹 앱과 달리, 당신의 마법은 실제 기기 기능에 액세스할 수 있습니다:
  📅 캘린더 • 🔔 알림 • 📍 위치 • 👤 연락처 • 🦿 헬스 커넥트 (Health Connect) • 🤖 AI • 🧭 센서 • 🔒 생체 인증 잠금 • 🏠 홈 화면 단축키 • 📷 카메라 & QR • 🗣️ TTS • 📳 햅틱 • 🎨 이미지 생성 • 🎤 음성 인식 • 🔋 배터리 • 🖨️ 인쇄
• **다른 앱 열기**: WhatsApp, Telegram, Notion, Spotify 등 딥 링크를 지원하는 모든 앱을 실행할 수 있습니다!
• **📥 마법으로 공유 (Share TO Spells)**: 다른 앱에서 파일을 직접 공유하여 즉시 처리하세요:
  📱 WhatsApp 대화 내보내기 → 핵심 주제 요약
  📄 PDF → 중요 데이터 추출
  🎤 오디오 → 텍스트 변환 및 분석
  📸 이미지 → 텍스트 추출 또는 보고서 생성
• **AI로 편집**: 바꾸고 싶은 부분이 있나요? "배경을 파란색으로 해줘" 또는 "리셋 버튼을 추가해줘"라고 말하면 AI가 실시간으로 업데이트합니다.
• **공유하기**: 당신의 창작물을 내보내어 친구들과 공유하세요. 데이터는 비공개로 유지됩니다.

**🔒 개인정보 우선:**
우리는 사용자의 소유권을 존중합니다. 당신의 마법은 당신의 기기에만 존재합니다. 우리는 데이터를 판매하거나 창작물에 대한 소유권을 주장하지 않습니다.

**지금 Appacadabra를 다운로드하고, 당신만의 마법서를 만들어 보세요!**


---

## Dutch (nl-NL)

### App Name (30 chars)
Appacadabra: AI Tool Generator

### Short Description (80 chars)
Zet je wens direct om in functionele tools. Geen code nodig, alleen AI.

### Full Description (4000 chars)
**Waar heb je hulp bij nodig?** Beschrijf het. De AI creëert het. 🪄

Appacadabra is een **AI Tool Generator** die jouw ideeën in seconden omzet in werkende tools. Geen codeerwerk nodig—beschrijf gewoon wat je nodig hebt en zie de magie gebeuren.

Een gewoontetracker nodig? Een trainingstimer? Een budgetcalculator? Een studieplanner? Een nieuwe game speciaal voor jou? Typ gewoon wat je wilt en de AI bouwt het direct voor je.

**✨ Belangrijkste Functies:**

• **Beschrijf & Creëer**: Typ gewoon "Een Pomodoro-timer met een ruimtethema" of "Een fooiencalculator voor groepen" en krijg een volledig functionele tool in seconden.
• **Persistent & Privé**: Je spreuken worden lokaal op je apparaat opgeslagen. Ze werken offline en zijn volledig van jou.
• **Native Apparaatkrachten**: In tegenstelling tot simpele web-apps, hebben jouw spreuken toegang tot echte apparaatfuncties:
  📅 Agenda • 🔔 Meldingen • 📍 Locatie • 👤 Contacten • 🦿 Health Connect • 🤖 AI • 🧭 Sensoren • 🔒 Biometrisch Slot • 🏠 Snelkoppelingen • 📷 Camera & QR • 🗣️ Spraak-naar-tekst • 📳 Haptiek • 🎨 Afb. Gen. • 🎤 Spraakherkenning • 🔋 Batterij • 🖨️ Afdrukken
• **Open Andere Apps**: Je spreuken kunnen WhatsApp, Telegram, Notion, Spotify openen—elke app die delen of deep links ondersteunt!
• **📥 Delen NAAR Spreuken**: Deel bestanden direct vanuit andere apps voor directe verwerking:
  📱 WhatsApp-exports → Vat belangrijke onderwerpen samen
  📄 PDF's → Haal belangrijke gegevens eruit
  🎤 Audio → Transcribeer & analyseer
  📸 Afbeeldingen → Haal tekst eruit of genereer rapporten
• **Bewerken met AI**: Iets veranderen? Zeg gewoon "Maak de achtergrond blauw" of "Voeg een resetknop toe," en de AI werkt het bij in real-time.
• **Deel Je Spreuken**: Exporteer je creaties om met vrienden te delen—je gegevens blijven privé.

**🔒 Privacy Eerst:**
Wij geloven in gebruikerseigendom. Je spreuken leven op je apparaat. We verkopen je gegevens niet door en claimen geen eigendom van je creaties.

**Download Appacadabra vandaag nog en begin met het bouwen van je eigen spreukenboek, één spreuk tegelijk!**

---

## Polish (pl-PL)

### App Name (30 chars)
Appacadabra: Generator Narzędzi AI

### Short Description (80 chars)
Zmień swoje pragnienie w funkcjonalne narzędzia natychmiast. Bez kodu, tylko AI.

### Full Description (4000 chars)
**W czym potrzebujesz pomocy?** Opisz to. AI to stworzy. 🪄

Appacadabra to **Generator Narzędzi AI**, który zamienia Twoje pomysły w działające narzędzia w kilka sekund. Nie musisz umieć kodować—po prostu opisz, czego potrzebujesz, i obserwuj magię.

Potrzebujesz trackera nawyków? Timera treningowego? Kalkulatora budżetu? Planera nauki? Nowej gry tylko dla Ciebie? Po prostu wpisz, czego chcesz, a AI zbuduje to natychmiast.

**✨ Kluczowe Funkcje:**

• **Opisz i Stwórz**: Wpisz "Timer Pomodoro z motywem kosmicznym" lub "Kalkulator napiwków dla grup" i otrzymaj w pełni funkcjonalne narzędzie w kilka sekund.
• **Trwałe i Prywatne**: Twoje zaklęcia są przechowywane lokalnie na Twoim urządzeniu. Działają offline i są całkowicie Twoje.
• **Natywne Moce Urządzenia**: W przeciwieństwie do zwykłych stron internetowych, Twoje zaklęcia mają dostęp do prawdziwych funkcji urządzenia:
  📅 Kalendarz • 🔔 Powiadomienia • 📍 Lokalizacja • 👤 Kontakty • 🦿 Health Connect • 🤖 AI • 🧭 Czujniki • 🔒 Blokada Biometryczna • 🏠 Skróty • 📷 Aparat & QR • 🗣️ TTS • 📳 Haptyka • 🎨 Gen. Obrazów • 🎤 Rozp. Mowy • 🔋 Bateria • 🖨️ Drukowanie
• **Otwieraj Inne Aplikacje**: Twoje zaklęcia mogą uruchamiać WhatsApp, Telegram, Notion, Spotify—każdą aplikację obsługującą udostępnianie lub linki głębokie!
• **📥 Udostępnij DO Zaklęć**: Udostępniaj pliki bezpośrednio z innych aplikacji do natychmiastowego przetworzenia:
  📱 Eksporty WhatsApp → Podsumuj kluczowe tematy
  📄 PDF-y → Wyciągnij ważne dane
  🎤 Audio → Transkrybuj i analizuj
  📸 Obrazy → Wyciągnij tekst lub generuj raporty
• **Edytuj z AI**: Chcesz coś zmienić? Po prostu powiedz "Zmień tło na niebieskie" lub "Dodaj przycisk reset", a AI zaktualizuje to w czasie rzeczywistym.
• **Udostępniaj Swoje Zaklęcia**: Eksportuj swoje dzieła, aby dzielić się nimi z przyjaciółmi—Twoje dane pozostają prywatne.

**🔒 Prywatność Przede Wszystkim:**
Wierzymy w własność użytkownika. Twoje zaklęcia żyją na Twoim urządzeniu. Nie sprzedajemy Twoich danych ani nie rości sobie praw do Twoich dzieł.

**Pobierz Appacadabra dzisiaj i zacznij budować własną księgę zaklęć, jedno zaklęcie na raz!**

---

## Vietnamese (vi-VN)

### App Name (30 chars)
Appacadabra: Máy Tạo Công Cụ AI

### Short Description (80 chars)
Biến mong muốn của bạn thành công cụ ngay lập tức. Không cần mã, chỉ cần AI.

### Full Description (4000 chars)
**Bạn cần giúp đỡ gì?** Mô tả nó. AI tạo ra nó. 🪄

Appacadabra là một **Máy Tạo Công Cụ AI** biến ý tưởng của bạn thành các tiện ích hoạt động trong vài giây. Không cần mã hóa—chỉ cần mô tả những gì bạn cần và xem phép thuật xảy ra.

Cần theo dõi thói quen? Hẹn giờ tập luyện? Máy tính ngân sách? Lập kế hoạch học tập? Một trò chơi mới chỉ dành cho bạn? Chỉ cần nhập những gì bạn muốn và AI sẽ xây dựng nó cho bạn ngay lập tức.

**✨ Các Tính Năng Chính:**

• **Mô Tả & Tạo**: Chỉ cần nhập "Hẹn giờ Pomodoro với chủ đề không gian" hoặc "Máy tính tiền boa cho nhóm" và nhận một công cụ đầy đủ chức năng trong vài giây.
• **Bền Vững & Riêng Tư**: Các phép thuật của bạn được lưu trữ cục bộ trên thiết bị. Chúng hoạt động ngoại tuyến và hoàn toàn là của bạn.
• **Sức Mạnh Thiết Bị Gốc**: Không giống như các trang web cơ bản, các phép thuật của bạn có thể truy cập các tính năng thực của thiết bị:
  📅 Lịch • 🔔 Thông báo • 📍 Vị trí • 👤 Danh bạ • 🦿 Health Connect • 🤖 AI • 🧭 Cảm biến • 🔒 Khóa sinh trắc học • 🏠 Phím tắt • 📷 Camera & QR • 🗣️ TTS • 📳 Rung • 🎨 Tạo Hình Ảnh • 🎤 Nhận dạng Giọng nói • 🔋 Pin • 🖨️ In ấn
• **Mở Các Ứng Dụng Khác**: Các phép thuật của bạn có thể khởi chạy WhatsApp, Telegram, Notion, Spotify—bất kỳ ứng dụng nào hỗ trợ chia sẻ hoặc liên kết sâu!
• **📥 Chia Sẻ ĐẾN Phép Thuật**: Chia sẻ tệp trực tiếp từ các ứng dụng khác để xử lý ngay lập tức:
  📱 Xuất WhatsApp → Tóm tắt các chủ đề chính
  📄 PDFs → Trích xuất dữ liệu quan trọng
  🎤 Âm thanh → Chuyển chép & phân tích
  📸 Hình ảnh → Trích xuất văn bản hoặc tạo báo cáo
• **Chỉnh Sửa với AI**: Bạn muốn thay đổi điều gì đó? Chỉ cần nói "Làm nền màu xanh" hoặc "Thêm nút đặt lại," và AI cập nhật nó trong thời gian thực.
• **Chia Sẻ Phép Thuật Của Bạn**: Xuất các sáng tạo của bạn để chia sẻ với bạn bè—dữ liệu của bạn luôn riêng tư.

**🔒 Riêng Tư Là Trên Hết:**
Chúng tôi tin vào quyền sở hữu của người dùng. Các phép thuật của bạn sống trên thiết bị của bạn. Chúng tôi không bán lại dữ liệu của bạn hoặc yêu cầu quyền sở hữu các sáng tạo của bạn.

**Tải xuống Appacadabra ngay hôm nay và bắt đầu xây dựng cuốn sách phép thuật của riêng bạn, từng phép thuật một!**

---

## Thai (th-TH)

### App Name (30 chars)
Appacadabra: ตัวสร้างเครื่องมือ AI

### Short Description (80 chars)
เปลี่ยนความต้องการของคุณเป็นเครื่องมือที่ใช้งานได้ทันที ไม่ต้องเขียนโค้ด แค่ใช้ AI

### Full Description (4000 chars)
**คุณต้องการความช่วยเหลือเรื่องอะไร?** อธิบายมัน AI จะสร้างให้ 🪄

Appacadabra คือ **ตัวสร้างเครื่องมือ AI** ที่เปลี่ยนไอเดียของคุณให้เป็นแอปพลิเคชันที่ใช้งานได้จริงในไม่กี่วินาที ไม่ต้องเขียนโค้ด—แค่อธิบายสิ่งที่คุณต้องการและดูเวทมนตร์เกิดขึ้น

ต้องการตัวติดตามนิสัย? ตัวจับเวลาออกกำลังกาย? เครื่องคำนวณงบประมาณ? แผนการเรียน? เกมใหม่สำหรับคุณโดยเฉพาะ? แค่พิมพ์สิ่งที่คุณต้องการและ AI จะสร้างให้คุณทันที

**✨ คุณสมบัติหลัก:**

• **อธิบาย & สร้าง**: แค่พิมพ์ "นาฬิกาจับเวลา Pomodoro ในธีมอวกาศ" หรือ "เครื่องคำนวณทิปสำหรับกลุ่ม" และรับเครื่องมือที่ใช้งานได้เต็มรูปแบบในไม่กี่วินาที
• **ถาวร & ส่วนตัว**: เวทมนตร์ของคุณถูกเก็บไว้ในอุปกรณ์ของคุณ ทำงานแบบออฟไลน์และเป็นของคุณโดยสมบูรณ์
• **พลังอุปกรณ์เนทีฟ**: ต่างจากเว็บบางๆ เวทมนตร์ของคุณสามารถเข้าถึงฟีเจอร์ของอุปกรณ์จริง:
  📅 ปฏิทิน • 🔔 การแจ้งเตือน • 📍 ตำแหน่ง • 👤 ผู้ติดต่อ • 🦿 Health Connect • 🤖 AI • 🧭 เซ็นเซอร์ • 🔒 ล็อคชีวภาพ • 🏠 ทางลัด • 📷 กล้อง & QR • 🗣️ TTS • 📳 การสั่น • 🎨 สร้างรูปภาพ • 🎤 จดจำเสียง • 🔋 แบตเตอรี่ • 🖨️ การพิมพ์
• **เปิดแอปอื่นๆ**: เวทมนตร์ของคุณสามารถเปิด WhatsApp, Telegram, Notion, Spotify—แอปใดๆ ที่รองรับการแชร์หรือ Deep link!
• **📥 แชร์ไปยังเวทมนตร์**: แชร์ไฟล์โดยตรงจากแอปอื่นเพื่อประมวลผลทันที:
  📱 ส่งออก WhatsApp → สรุปหัวข้อสำคัญ
  📄 PDFs → ดึงข้อมูลสำคัญ
  🎤 เสียง → ถอดความ & วิเคราะห์
  📸 รูปภาพ → ดึงข้อความหรือสร้างรายงาน
• **แก้ไขด้วย AI**: ต้องการเปลี่ยนอะไรไหม? แค่บอก "เปลี่ยนพื้นหลังเป็นสีน้ำเงิน" หรือ "เพิ่มปุ่มรีเซ็ต" และ AI จะอัปเดตแบบเรียลไทม์
• **แชร์เวทมนตร์ของคุณ**: ส่งออกผลงานของคุณเพื่อแชร์กับเพื่อน—ข้อมูลของคุณเป็นส่วนตัว

**🔒 ความเป็นส่วนตัวต้องมาก่อน:**
เราเชื่อในความเป็นเจ้าของของผู้ใช้ เวทมนตร์ของคุณอยู่บนอุปกรณ์ของคุณ เราไม่ขายข้อมูลของคุณหรืออ้างสิทธิ์ในผลงานของคุณ

**ดาวน์โหลด Appacadabra วันนี้และเริ่มสร้างหนังสือเวทมนตร์ของคุณเอง ทีละบท!**

---

## Arabic (ar-SA)

### App Name (30 chars)
Appacadabra: مولد أدوات بالذكاء الاصطناعي

### Short Description (80 chars)
حول رغبتك إلى أدوات فعالة فوراً. لا حاجة للكود، فقط الذكاء الاصطناعي.

### Full Description (4000 chars)
**بماذا تحتاج للمساعدة؟** صف ذلك. الذكاء الاصطناعي ينشئه. 🪄

Appacadabra هو **مولد أدوات بالذكاء الاصطناعي** يحول أفكارك إلى أدوات مساعدة تعمل خلال ثوانٍ. لا حاجة للبرمجة—فقط صف ما تحتاجه وشاهد السحر يحدث.

هل تحتاج إلى متتبع للعادات؟ مؤقت للتمارين؟ حاسبة للميزانية؟ مخطط للدراسة؟ لعبة جديدة خصيصًا لك؟ فقط اكتب ما تريده وسيبنيه الذكاء الاصطناعي لك فوراً.

**✨ الميزات الرئيسية:**

• **صف وأنشئ**: فقط اكتب "مؤقت بومودورو بطابع فضائي" أو "حاسبة إكرامية للمجموعات" واحصل على أداة كاملة الوظائف في ثوانٍ.
• **دائم ومستقل**: تعويذاتك مخزنة محلياً على جهازك. تعمل بدون اتصال بالإنترنت وهي ملكك بالكامل.
• **قوى الجهاز الأصلية**: على عكس تطبيقات الويب البسيطة، يمكن لتعويذاتك الوصول إلى ميزات الجهاز الحقيقية:
  📅 التقويم • 🔔 الإشعارات • 📍 الموقع • 👤 جهات الاتصال • 🦿 Health Connect • 🤖 الذكاء الاصطناعي • 🧭 أجهزة الاستشعار • 🔒 القفل البيومتري • 🏠 الاختصارات • 📷 الكاميرا & QR • 🗣️ تحويل النص لكلام • 📳 لمس • 🎨 توليد الصور • 🎤 التعرف على الكلام • 🔋 البطارية • 🖨️ الطباعة
• **فتح تطبيقات أخرى**: يمكن لتعويذاتك تشغيل WhatsApp، Telegram، Notion، Spotify—أي تطبيق يدعم المشاركة أو الروابط العميقة!
• **📥 شارك إلى التعويذات**: شارك الملفات مباشرة من تطبيقات أخرى للمعالجة الفورية:
  📱 تصديرات WhatsApp ← تلخيص الموضوعات الرئيسية
  📄 ملفات PDF ← استخراج البيانات المهمة
  🎤 الصوت ← نسخ وتحليل
  📸 الصور ← استخراج النص أو إنشاء التقارير
• **عدل بالذكاء الاصطناعي**: تريد تغيير شيء ما؟ فقط قل "اجعل الخلفية زرقاء" أو "أضف زر إعادة تعيين"، وسيقوم الذكاء الاصطناعي بتحديثه في الوقت الفعلي.
• **شارك تعويذاتك**: قم بتصدير إبداعاتك لمشاركتها مع الأصدقاء—بياناتك تظل خاصة.

**🔒 الخصوصية أولاً:**
نحن نؤمن بملكية المستخدم. تعويذاتك تعيش على جهازك. نحن لا نعيد بيع بياناتك أو ندعي ملكية إبداعاتك.

**حمل Appacadabra اليوم وابدأ في بناء كتاب التعويذات الخاص بك، تعويذة تلو الأخرى!**

---

## Hindi (hi-IN)

### App Name (30 chars)
Appacadabra: AI टूल जेनरेटर

### Short Description (80 chars)
अपनी इच्छा को तुरंत कार्यात्मक टूल में बदलें। कोई कोड जरूरी नहीं, बस AI।

### Full Description (4000 chars)
**आपको किस चीज़ में मदद चाहिए?** इसका वर्णन करें। AI इसे बनाता है। 🪄

Appacadabra एक **AI टूल जेनरेटर** है जो आपके विचारों को सेकंडों में काम करने वाली उपयोगिताओं में बदल देता है। कोडिंग की आवश्यकता नहीं है—बस वर्णन करें कि आपको क्या चाहिए और जादू देखें।

क्या आपको आदत ट्रैकर चाहिए? वर्कआउट टाइमर? बजट कैलकुलेटर? अध्ययन योजनाकार? सिर्फ आपके लिए एक नया गेम? बस टाइप करें कि आप क्या चाहते हैं और AI इसे आपके लिए तुरंत बना देता है।

**✨ मुख्य विशेषताएं:**

• **वर्णन करें और बनाएं**: बस टाइप करें "एक अंतरिक्ष थीम के साथ पोमोडोरो टाइमर" या "समूहों के लिए टिप कैलकुलेटर" और सेकंडों में पूरी तरह से कार्यात्मक टूल प्राप्त करें।
• **स्थायी और निजी**: आपके मंत्र आपके डिवाइस पर स्थानीय रूप से संग्रहीत होते हैं। वे ऑफ़लाइन काम करते हैं और पूरी तरह से आपके हैं।
• **मूल डिवाइस शक्तियां**: बुनियादी वेब रैपर के विपरीत, आपके मंत्र वास्तविक डिवाइस सुविधाओं तक पहुंच सकते हैं:
  📅 कैलेंडर • 🔔 सूचनाएं • 📍 स्थान • 👤 संपर्क • 🦿 हेल्थ कनेक्ट • 🤖 AI • 🧭 सेंसर • 🔒 बायोमेट्रिक लॉक • 🏠 शॉर्टकट • 📷 कैमरा और QR • 🗣️ TTS • 📳 हैप्टिक्स • 🎨 छवि जनरेशन • 🎤 वाक पहचान • 🔋 बैटरी • 🖨️ प्रिंटिंग
• **अन्य ऐप्स खोलें**: आपके मंत्र WhatsApp, Telegram, Notion, Spotify—साझाकरण या गहरे लिंक का समर्थन करने वाले किसी भी ऐप को लॉन्च कर सकते हैं!
• **📥 मंत्रों को साझा करें**: तत्काल प्रसंस्करण के लिए अन्य ऐप्स से सीधे फाइलें साझा करें:
  📱 WhatsApp निर्यात → मुख्य विषयों का सारांश बनाएं
  📄 PDF → महत्वपूर्ण डेटा निकालें
  🎤 ऑडियो → प्रतिलेखन और विश्लेषण करें
  📸 चित्र → पाठ निकालें या रिपोर्ट बनाएं
• **AI के साथ संपादित करें**: कुछ बदलना चाहते हैं? बस कहें "पृष्ठभूमि को नीला करें" या "एक रीसेट बटन जोड़ें," और AI इसे वास्तविक समय में अपडेट करता है।
• **अपने मंत्र साझा करें**: दोस्तों के साथ साझा करने के लिए अपनी कृतियों को निर्यात करें—आपका डेटा निजी रहता है।

**🔒 गोपनीयता पहले:**
हम उपयोगकर्ता स्वामित्व में विश्वास करते हैं। आपके मंत्र आपके डिवाइस पर रहते हैं। हम आपका डेटा नहीं बेचते हैं या आपकी कृतियों के स्वामित्व का दावा नहीं करते हैं।

**आज ही Appacadabra डाउनलोड करें और अपनी खुद की स्पेल बुक बनाना शुरू करें, एक समय में एक मंत्र!**

---

## Russian (ru-RU)

### App Name (30 chars)
Appacadabra: Генератор ИИ-инструментов

### Short Description (80 chars)
Мгновенно превращайте желания в рабочие инструменты. Без кода, только ИИ.

### Full Description (4000 chars)
**С чем вам нужна помощь?** Опишите это. ИИ создаст это. 🪄

Appacadabra — это **Генератор ИИ-инструментов**, который превращает ваши идеи в рабочие утилиты за секунды. Программирование не требуется — просто опишите, что вам нужно, и смотрите, как происходит магия.

Нужен трекер привычек? Таймер тренировок? Калькулятор бюджета? Планировщик учебы? Новая игра только для вас? Просто напишите, что вы хотите, и ИИ создаст это для вас мгновенно.

**✨ Ключевые особенности:**

• **Опиши и Создай**: Просто напишите "Таймер Pomodoro в космическом стиле" или "Калькулятор чаевых для групп" и получите полностью функциональный инструмент за секунды.
• **Постоянные и Приватные**: Ваши заклинания хранятся локально на вашем устройстве. Они работают офлайн и полностью принадлежат вам.
• **Нативные возможности устройства**: В отличие от простых веб-оболочек, ваши заклинания могут использовать реальные функции устройства:
  📅 Календарь • 🔔 Уведомления • 📍 Местоположение • 👤 Контакты • 🦿 Health Connect • 🤖 ИИ • 🧭 Датчики • 🔒 Биометрический замок • 🏠 Ярлыки • 📷 Камера и QR • 🗣️ TTS • 📳 Гаптика • 🎨 Генерация изобр. • 🎤 Распознавание речи • 🔋 Батарея • 🖨️ Печать
• **Открывайте другие приложения**: Ваши заклинания могут запускать WhatsApp, Telegram, Notion, Spotify — любое приложение, поддерживающее обмен данными или глубокие ссылки!
• **📥 Поделиться В Заклинания**: Делитесь файлами напрямую из других приложений для мгновенной обработки:
  📱 Экспорт WhatsApp → Суммирование ключевых тем
  📄 PDF → Извлечение важных данных
  🎤 Аудио → Транскрибация и анализ
  📸 Изображения → Извлечение текста или создание отчетов
• **Редактируйте с ИИ**: Хотите что-то изменить? Просто скажите "Сделай фон синим" или "Добавь кнопку сброса", и ИИ обновит это в реальном времени.
• **Делитесь своими заклинаниями**: Экспортируйте свои творения, чтобы поделиться с друзьями — ваши данные остаются приватными.

**🔒 Конфиденциальность прежде всего:**
Мы верим в право собственности пользователя. Ваши заклинания живут на вашем устройстве. Мы не перепродаем ваши данные и не претендуем на ваши творения.

**Скачайте Appacadabra сегодня и начните создавать свою собственную книгу заклинаний, по одному за раз!**

---

## Turkish (tr-TR)

### App Name (30 chars)
Appacadabra: AI Araç Oluşturucu

### Short Description (80 chars)
İsteğinizi anında işlevsel araçlara dönüştürün. Kod yok, sadece yapay zeka.

### Full Description (4000 chars)
**Neye ihtiyacınız var?** Tarif edin. Yapay zeka yaratsın. 🪄

Appacadabra, fikirlerinizi saniyeler içinde çalışan araçlara dönüştüren bir **Yapay Zeka Araç Oluşturucusudur**. Kodlama gerekmez—sadece neye ihtiyacınız olduğunu tarif edin ve sihrin gerçekleşmesini izleyin.

Alışkanlık takipçisi mi lazım? Egzersiz zamanlayıcısı? Bütçe hesaplayıcı? Çalışma planlayıcısı? Sadece sizin için yeni bir oyun mu? İstediğinizi yazın ve AI sizin için anında oluştursun.

**✨ Temel Özellikler:**

• **Tarif Et ve Oluştur**: Sadece "Uzay temalı bir Pomodoro zamanlayıcısı" veya "Gruplar için bahşiş hesaplayıcısı" yazın ve saniyeler içinde tam işlevsel bir araç edinin.
• **Kalıcı ve Özel**: Büyüleriniz cihazınızda yerel olarak saklanır. Çevrimdışı çalışırlar ve tamamen sizindirler.
• **Yerel Cihaz Güçleri**: Basit web sarmalayıcılarının aksine, büyüleriniz gerçek cihaz özelliklerine erişebilir:
  📅 Takvim • 🔔 Bildirimler • 📍 Konum • 👤 Kişiler • 🦿 Sağlık • 🤖 AI • 🧭 Sensörler • 🔒 Biyometrik Kilit • 🏠 Kısayollar • 📷 Kamera ve QR • 🗣️ Metin Okuma • 📳 Titreşim • 🎨 Görsel Oluşturma • 🎤 Konuşma Tanıma • 🔋 Pil • 🖨️ Yazdırma
• **Diğer Uygulamaları Aç**: Büyüleriniz WhatsApp, Telegram, Notion, Spotify'ı başlatabilir—paylaşımı veya derin bağlantıları destekleyen herhangi bir uygulamayı!
• **📥 Büyülere Paylaş**: Anında işleme için diğer uygulamalardan dosyaları doğrudan paylaşın:
  📱 WhatsApp dışa aktarımları → Önemli konuları özetle
  📄 PDF'ler → Önemli verileri çıkar
  🎤 Ses → Deşifre et ve analiz et
  📸 Resimler → Metin çıkar veya rapor oluştur
• **AI ile Düzenle**: Bir şeyi değiştirmek mi istiyorsunuz? Sadece "Arka planı mavi yap" veya "Sıfırlama düğmesi ekle" deyin ve AI gerçek zamanlı olarak güncellesin.
• **Büyülerinizi Paylaşın**: Arkadaşlarınızla paylaşmak için kreasyonlarınızı dışa aktarın—verileriniz gizli kalır.

**🔒 Önce Gizlilik:**
Kullanıcı sahipliğine inanıyoruz. Büyüleriniz cihazınızda yaşar. Verilerinizi satmıyoruz veya kreasyonlarınız üzerinde hak iddia etmiyoruz.

**Appacadabra'yı bugün indirin ve kendi büyü kitabınızı oluşturmaya başlayın, her seferinde bir büyü!**
