<!-- 
  ...
  =========================
  Title: The Appacadabra Chronicles, Part 10: The Marketing Department -- Making the Product Visible
  Subtitle: How a solo technical founder built a content engine that sounds human -- and why the series itself is the strategy.
  Source: docs/blog/blog_expanded_part9_marketing.md
...
**[INSERT IMAGE: marketing_flow_part10.png]**
...
- A **Post Draft skill** (`/draft-post`): given a topic and target platform (X or LinkedIn), produces a complete draft in Appacadabra's voice. For X: a thread structure with hook, body tweets, and a close. For LinkedIn: a hook paragraph, narrative body with a specific insight or lesson, and a call-to-action. Always uses the magic/spell vocabulary where natural, never where forced.

- A **Content Calendar skill** (`/content-plan`): given a time period and any upcoming product milestones, produces a content calendar mapping events to formats -- which articles to publish when, which milestones to highlight on which platform, and the optimal cadence for each channel.

- A **Platform Adaptation skill** (`/adapt-post`): given a piece of existing content (an article excerpt, a feature announcement, a LinkedIn post), adapts it for a different platform -- compressing a LinkedIn article into an X thread, or expanding an X thread into a LinkedIn narrative -- while preserving the core insight and voice.

- A **Substack Publish skill** (`/substack-publish`): given a blog markdown file from `docs/blog/`, transforms it into a Substack-ready format -- converting Mermaid diagrams into image placeholders, adjusting heading levels, extracting the post title and subtitle, and writing the output to `docs/blog/substack/`. The last mile of the content pipeline: from internal documentation to public article, without manual reformatting.
