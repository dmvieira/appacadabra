...
async function main(): Promise<void> {
    ...
    // Active capabilities only (kept for the permission sync log)
    ...
    // Phase 4 (3.0.0 BYOK): the Firebase Functions AI pipeline is gone, so we no
    // longer emit `firebase/functions/src/capabilities/*.ts`. `lib/capabilities/`
    // is the single source of truth for docs (injected into the OpenRouter system
    // prompt client-side via `lib/api/systemPrompt.ts`). Only the Android side of
    // the sync (permissions + manifest blocks) is still needed.
